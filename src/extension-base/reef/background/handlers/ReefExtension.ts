...
import Extension from "../../../background/handlers/Extension";
import {
  createSubscription,
  unsubscribe,
} from "../../../background/handlers/subscriptions";
import {
  AccountJson,
  MessageTypes,
...
export const networkRpcUrlSubject: BehaviorSubject<string> =
  new BehaviorSubject<string>(
    localStorage.getItem(REEF_NETWORK_RPC_URL_KEY) || rpcUrl
  );
...
export default class ReefExtension extends Extension {
  override async handle<TMessageType extends MessageTypes>(
    id: string,
    type: TMessageType,
